...
database_has_station_status <- citibike_trip_db %>% dbExistsTable("station_status_new")
...
local_dir <- here("data/station_status/raw/old")
...
repeat {
    ...
    if (nrow(station_status_parsed) > 0) {
        ...
        cat("- - - - - - - - - - - - - - - - - - - - -\n")
        cat("Y:", unique(station_status$year), "M:", unique(station_status$month))
        cat("\n")
...
if (all(index_tbl$tbl_name != "station_status_new")) {
    
    citibike_trip_db %>% db_create_index("station_status_new", "station_id")
    citibike_trip_db %>% db_create_index("station_status_new", "last_reported")
    ...
    citibike_trip_db %>% db_create_index("station_status_new", "year")
    citibike_trip_db %>% db_create_index("station_status_new", "month")
    citibike_trip_db %>% db_create_index("station_status_new", "day")
    citibike_trip_db %>% db_create_index("station_status_new", "hour")
    citibike_trip_db %>% db_create_index("station_status_new", "minute")
